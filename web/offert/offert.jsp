...
<html>
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
                <title><%= t.t("Offert")%> <%= offertnr!=null ? offertnr : "" %></title>
...
<%
    ...
    ResultSet rsO1 = stm.executeQuery("select o1.*, s.tel, s.epost from " + dbPrefix + "offert1 o1 left outer join " + dbPrefix + "saljare s on s.namn=o1.saljare where offertnr=" + offertnr);
    if (rsO1.next()) {
%>
...
<div id="offer-header" style="width: 100%">
    ...
    <table style="width: 100%">
        <tr style="height: 80px;">
            ...
            <td style="width: 50%">
                <span style="font-size: 200%; font-weight:bold"><%= t.t("Offert") %> <%= rsO1.getInt("offertnr") %></span><br>
...
<%
         ...
         ResultSet rsO2 = stm.executeQuery("select o2.*, case when coalesce(a.bildartnr,'')='' then a.nummer else bildartnr end as bildnr  from " + dbPrefix + "offert2 o2 left outer join " + dbPrefix + "artikel a on a.nummer=o2.artnr where offertnr=" + offertnr + " order by pos"); %>
...
<table style="margin-top: 12px; width: 100%; border-top: 1px solid black; border-bottom: 1px solid black; padding-top: 2px; padding-bottom: 2px;">
    <tr><th style="width: 60px;"></th><th style="width: 98px"><%= t.t("Artikelnr") %></th><th style="width: 218px"><%= t.t("Benämning") %></th><th style="width: 86px; text-align: right;"><%= t.t("Antal") %></th><th style="width: 40px;"></th><th style="text-align: right; width: 62px;"><%= t.t("Pris") %></th><th style="width: 62px; padding-left: 4px;">%</th><th style="text-align: right; width: 72px"><%= t.t("Summa") %></th></tr>
...
<%
    ResultSet rs = stm.executeQuery("select offertnr, datum, namn from " + dbPrefix + "offert1 order by offertnr desc limit 30");
    while (rs.next()) {
%>    
<a href="?offertnr=<%= rs.getInt("offertnr") %>&land=<%= t.getSprak() %>">    
