...
public class UpdateAutonettoRunnable implements Runnable {
    ...
    public void doTask() throws InfoException {
        Connection con=null;
        Logger.getLogger("sx-logger").info("sxas.UpdateAutonettoRunnable start");
        try {
            con=Const.getSxAdmConnectionFromInitialContext();
            Statement stm = con.createStatement();
            stm.setQueryTimeout(60);
            stm.execute("select sxasfakt.autonettoupdate()");
        } catch(Exception e) {
            Logger.getLogger("sx-logger").info("sxas.UpdateAutonettoRunnable Fel:: " + e.getMessage()); 
            e.printStackTrace();
            throw new InfoException("Fel: " + e.getMessage());
        }
        finally {
            try { con.close(); } catch (Exception e) {}
            Logger.getLogger("sx-logger").info("sxas.UpdateAutonettoRunnable klar");
        }
    }
